...
class OportunidadUsuario:
    @staticmethod
    def get_oportunidad_by_codigo(codigo_oportunidad):
        try:
            ...
            if response:
                ...
                if isinstance(response, list) and len(response) > 0:
                    ...
                    print(f"No se encontró la oportunidad con código: {codigo_oportunidad}")
                    return None
            else:
                print("Error: la respuesta de la API es vacía")
                return None
        except requests.exceptions.RequestException as e:
            print(f"Error al obtener la oportunidad: {e}")
            return None
        except Exception as e:
    ...
    @staticmethod
    def insert_oportunidad_and_associate_user(form, user_email):
        """
        Inserta una nueva oportunidad en la API y asocia al usuario con la oportunidad.
        """
        ...
        try:
            ...
            # Crear los datos para la API para la 'oportunidad'
            json_data = {
            ...
            client = APIClient('oportunidad')  # Inicialización del cliente API para 'oportunidad'

            # Intentar insertar los datos en la API para la oportunidad
            response = client.insert_data(json_data=json_data)

            if response and 'codigo_oportunidad' in response:
                oportunidad_codigo = response['codigo_oportunidad']
                json_association = {
                    'email_usuario': user_email,
                    'codigo_oportunidad': oportunidad_codigo
                }

                # Asociar la oportunidad con el usuario
                association_response = client.insert_data(json_data=json_association)

                return True, 'Oportunidad creada y asociada exitosamente.'
            else:
                return False, 'Error al crear la oportunidad. La respuesta de la API no es válida.'
        except requests.exceptions.RequestException as e:
